/*
• You are given two numbers stored in two variables, with the name, num1 and num2
• Print the result of the following operations
1. num1 > num2 -> this operation prints true, if num1 is greater than num2, otherwise it prints false
2. num1 < num2 -> this operations prints true, if num is smaller than num2, otherwise it prints false
3. num1 == num2 -> this operation print true, if num is equal to num2, otherwise it prints false
• You have to print the result of three operations in the order as shown above
*/

function compareTwoNumbers(num1, num2, num3) {
    // Write code here  
    if(num1 > num2){
        console.log(true);
    }else{
       console.log(false);
    }
    
    
    if(num1 < num2){
       console.log(true);
    }else{
        console.log(false);
    }

    if(num1 === num2){
        console.log(true);
     }else{
         console.log(false);
     }
}
compareTwoNumbers(4,16);
